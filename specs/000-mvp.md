...
- [ ] **Setup Supabase email auth**
  ...
  - [ ] Redirect from `/` to `/dashboard` for signed-in users
  - [ ] Associate authenticated users with our user table in DB
  - [ ] Use `Button` component in Nav Bar
  - [ ] Test sign-on in preview environment
