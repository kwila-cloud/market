...
- [x] **Build signup flow**
  - [x] To avoid leaking details about existing users, we will create new user for each new OTP sign up
  - [x] Add column to users table that tracks their auth user ID (not nullable)
  - [x] Add invite code entry page for users that have signed in but don't have a record in the users table yet
    - [x] Prompt them to enter invite code
    - [x] Do not allow them to view any other page in the frontend until their enter invite code
  - [x] Update login page with appropriate warning
    - [x] Use "Welcome" instead of "Welcome back"
    - [x] Remove message about needing an invite code to sign up
  - [x] We only create new record in the users table once a user has entered a valid invite code
    - [x] We will need to remove current trigger that automatically adds records to the users table
    - [x] Users that are not in the users table should not be allowed to create new items, threads, or connections (enforced by RLS)
  - [x] Once user inputs valid invite code, use Supabase RPC function to set things up:
    - [x] Create user record with invited_by relationship
    - [x] Add new accepted connection between two users
  - [x] Build basic onboarding flow
    - [x] Contact visibility settings
    - [x] Display Name (default to value from invite code)
    - [x] Bio information
    - [x] For now - use https://www.dicebear.com/styles/initials/ to automatically generate and upload avatar for new user
  - [ ] Update SECURITY.md
  - [ ] Add validation to ensure about info is NOT more than 500 characters
  - [ ] Figure out why complete signup button isn't doing anything
  - [ ] Add loading indicator while `complete_signup` is running
...
- [ ] **Add error handling and user feedback**
  ...
  - [x] Consistent error styling
    - [x] Use error color scheme rather than hardcoded red color scheme
    - [x] Shared components for error alerts
      - [x] Login form
      - [x] Invite manager
      - [x] OTP form
      - [x] Signup form
